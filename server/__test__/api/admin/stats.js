...
describe('Testing for get host profile route', () => {
  ...
  test('test with correct user id and intern', done => {
    ...
    const { adminUser, internUser4 } = users;
    const { financialTimeOrganisation } = organisations;
    ...
    request(app)
      ...
      .end((error, result) => {
        ...
        expect(result.body[0].name).toBe(internUser4.name);
  ...
  test('test with correct user id and host', done => {
    ...
    request(app)
      ...
      .end((error, result) => {
        ...
        expect(result.body[0].totalIncome).toBe(49500);
        expect(result.body[0].currentBalance).toBe(49500);
        done(error);
      });
  });
});
